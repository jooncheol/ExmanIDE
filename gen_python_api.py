...
word.characters.*.py=$(chars.alpha)$(chars.numeric)_
...
modify for ExmanIDE
by Park joon cheol (jooncheol@gmail.com)
'''
...
excludedirlist = ['lib-tk', 'Lightflow', 'plat-linux-i386',
                  'win32', 'pythonwin', 'plat-win',
                  'test', 'distutils', 'encodings', 'OpenGL', 'gnome', 'pyglade',
                  'happydoc', 'pygame', 'mx','wxPython','qt','japanese','idle']
        ...
        if ns['doc']:
            if pattern.search(ns['doc']):
                if not api.has_key(entryprefix + name):
                    #api[entryprefix + name] = '|'+string.strip(string.split(ns['doc'], '\n')[0]) + '\n'
                    api[entryprefix + name] = '|'+string.strip(ns['doc']) + '\n'
                    
                    return
            else:
                if ns['nametype'] in [types.ClassType, types.FunctionType]:
                    #api[entryprefix + name] = '(??)| [doc: ' + string.strip(string.split(ns['doc'], '\n')[0]) + ']' + '\n'
                    api[entryprefix + name] = '(??)| [doc: ' + string.strip(ns['doc']) + ']' + '\n'
                    

    if not api.has_key(entryprefix + name):
        if ns['nametype'] == types.ClassType:
            api[entryprefix + name] = '(??)| [class]\n'
        elif ns['nametype'] == types.FunctionType:
            api[entryprefix + name] = '(??)| [function]\n'
        elif ns['nametype'] == types.ModuleType:
            api[entryprefix + name] = '| [module]\n'
        else:
            api[entryprefix + name] = '|\n'
        ...
        if def_m:
            ...
            if api.has_key(entryprefix + name):
                ...
                if docindex + 1:
                    ...
                    api[entryprefix + name] = '|' + def_m.group(2) + doc
                if api[entryprefix + name] == '(??)| [function]\n':
                    api[entryprefix + name] = '|' + def_m.group(2) + '\n'
            ...
            if init_m:
                if api.has_key(entryprefix + classname):
                    ...
                    if docindex + 1:
                        ...
                        api[entryprefix + classname] = '(' + init_m.group(1) + ')|' + doc

                    if api[entryprefix + classname] == '(??)| [class]\n':
                        api[entryprefix + classname] = '(' + init_m.group(1) + ')|' + '\n'
                inclass = 0
            if not line[0] in ' \t\n':
                inclass = 0
